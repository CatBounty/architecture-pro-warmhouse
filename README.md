...
![er_diagramm.svg](schemas/erd.svg)

[ER-диаграмма (PlantUML)](schemas/erd.puml)

# Задание 4. Создание и документирование API

### 1. Тип API

* **AsyncAPI** - для управления устройствами и получения телеметрии по протоколу **MQTT**
  * Промышленный стандарт
  * Решает вопросы с организацией доступа к хабам умного дома из внешних сетей
  * Позволяет гибко масштабировать нагрузку
  * Не требует балансировщиков нагрузки, ограничений лимитов на запросы, Circuit Breaker.
* **AsyncAPI** - для межсервисного взаимодействия. Брокер **RabbitMQ**.
  * Уменьшает связанность систем друг с другом
  * Позволяет масштабировать отдельные сервисы
* **REST API** - для интерфейса управления через Web UI
  * Позволяет получать ответ синхронно
  * Легко использовать в клиентских веб-приложениях (Angular)

### 2. Документация API
* [REST API Device Management Service](schemas/device_management_service_api.yaml)
* [REST API Device Monitoring Service](schemas/device_monitoring_service_api.yaml)
* [REST API Device Control Service](schemas/device_control_service_api.yaml)
